...
<div class="panel">
    <?php foreach ($orders as $order): ?>
        <div class="displayName">
            <span><?= $order->orderedby ?> <span style="cursor: pointer;" data-name="<?= h($order->orderedby) ?>" data-ordered="<?= h($order->hirsch->name) ?>" data-special="<?= h($order->note) ?>" onclick="$('#orderedModalTitle').html('Bestellung von '+$(this).attr('data-name'));$('#orderedModalText').html($(this).attr('data-ordered')+(($(this).attr('data-special')!='')?'<br><br>'+$(this).attr('data-special'):''));$('#orderedModal').addClass('active')">🍚</span> </span> <?= (isset($_COOKIE['lastOrderedName']) && $order->orderedby == \Cake\Utility\Security::decrypt($_COOKIE['lastOrderedName'], 'ordererNameDecryptionKeyLongVersion')) ? $this->Form->postLink('<i class="material-icons">delete_forever</i>', ['controller' => 'orders', 'action' => 'delete', base64_encode($order->id)], ['confirm' => 'Bist du sicher, dass du diese Bestellung löschen willst?', 'class' => 'btn', 'escape' => false, 'disabled' => (new Time())->hour >= 11]) : "" ?>
        </div>
    <?php endforeach; ?>
</div>
